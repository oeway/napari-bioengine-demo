...
def get_config(server_url, model_name):
    response = requests.get(
        f"{server_url}/public/services/triton-client/get_config?model_name="
        + model_name,
    )
...
def execute(inputs, server_url, model_name, **kwargs):
    ...
    if response.ok:
        ...
        raise Exception(
            f"Failed to execute {model_name}: {response.reason or response.text}"
        )


if __name__ == "__main__":
    # Get the model config with information about inputs/outputs etc.
    config = get_config("https://ai.imjoy.io", "cell-python")
    # print(config)
    ...
    image_array = np.random.randint(
        0, 255, size=(1, 3, 128, 128), dtype=np.uint8
    )
    ...
    print("Mask predicted: ", mask.shape)
    assert mask.shape == (1, 3, 128, 128), str(mask.shape)
